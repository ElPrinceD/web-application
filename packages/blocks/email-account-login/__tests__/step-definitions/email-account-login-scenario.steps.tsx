...
defineFeature(feature, (test) => {
    ...
    test('User navigates to Email Log In', ({ given, when, then }) => {
        ...
        then('I can select the Log In button with out errors', () => {
            instance.goToSocialLogin = jest.fn();
            let buttonComponent = mobileAccountLogInWrapper.findWhere((node) => node.prop('testID') === 'btnEmailLogIn');
            buttonComponent.simulate('press');
            const { onPress } = instance.btnSocialLoginProps;
    onPress();
    expect(instance.goToSocialLogin).toHaveBeenCalled();
    instance.goToSocialLogin()
    const expectedMessage = new Message(getName(MessageEnum.NavigationSocialLogInMessage));
    expectedMessage.addData(getName(MessageEnum.NavigationPropsMessage), expectedMessage.messageId);
    runEngine.sendMessage("Unit Test", expectedMessage)
